...
By default, the job runner periodically checks for updated files via a timestamp manifest (`{S3_INDEX_PREFIX}/manifest/timestamp_v1.json`) and syncs only changed entries. This can be configured via environment variables (see `.env.example`).
